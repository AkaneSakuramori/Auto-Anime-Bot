...
from bot.core.auto_animes import get_animes, process_file
    ...
    if len(txtargs) <= 1:
        ...
        for elem in Var.START_BUTTONS.split():
            try:
                ...
                continue
        smsg = Var.START_MSG.format(first_name=from_user.first_name,last_name=from_user.first_name,mention=from_user.mention,user_id=from_user.id)
        if Var.START_PHOTO:
            await message.reply_photo(photo=Var.START_PHOTO,caption=smsg,reply_markup=InlineKeyboardMarkup(btns) if btns else None)
        else:
...
async def process_direct_file(client, message):
    if not message.document:
        return await sendMessage(message, "⚠️ Please send a valid video file (.mkv or .mp4)")
    ...
    if filename.endswith(('.mkv', '.mp4')):
        ...
        try:
            ...
            await temp.edit(f"❌ Error while processing file:\n<code>{e}</code>")
            await rep.report(f"process_file() failed: {e}", "error")
    else:
        await sendMessage(message, "⚠️ Please send a valid video file (.mkv or .mp4)")
...
async def add_task(client, message):
    ...
    if not (taskInfo := await getfeed(args[1], index)):
        return await sendMessage(message, "<b>No Task Found for Provided Link</b>")
