...
from bot.core.auto_animes import get_animes, process_file  # ✅ fixed import
...
async def process_direct_file(client, message):
    """Handles admin-uploaded video files directly."""
    if message.document and message.document.file_name.endswith(('.mkv', '.mp4')):
        temp = await sendMessage(message, "<i>Processing file...</i>")
        try:
            await process_file(message)  # ✅ properly imported from auto_animes
